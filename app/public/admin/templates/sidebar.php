<div class="sidebar min-vh-100 col col-md-2 position-relative bg-body-tertiary border-end" style="top: 47px">
    <nav class="sidebar-nav mt-2">
        <ul class="nav nav-pills flex-column ps-0 list-unstyled">
            <li class="nav-item">
                <a href="articles.php" class="nav-link text-dark-emphasis">

                    <svg xmlns="http://www.w3.org/2000/svg" height="18" viewBox="0 96 960 960" width="21">
                        <use href="#article-icon"></use>
                    </svg>

                    കുറിപ്പുകൾ
                </a>
            </li>
            <li class="nav-item">
                <a href="image-library.php" class="nav-link text-dark-emphasis">
                    <svg xmlns="http://www.w3.org/2000/svg" height="18" viewBox="0 96 960 960" width="21"><path d="M479.5 790q72.5 0 121.5-49t49-121.5q0-72.5-49-121T479.5 450q-72.5 0-121 48.5t-48.5 121q0 72.5 48.5 121.5t121 49Zm0-60q-47.5 0-78.5-31.5t-31-79q0-47.5 31-78.5t78.5-31q47.5 0 79 31t31.5 78.5q0 47.5-31.5 79t-79 31.5ZM140 936q-24 0-42-18t-18-42V363q0-23 18-41.5t42-18.5h147l73-87h240l73 87h147q23 0 41.5 18.5T880 363v513q0 24-18.5 42T820 936H140Zm680-60V363H645l-73-87H388l-73 87H140v513h680ZM480 620Z"/></svg>

                    ചിത്രങ്ങൾ
                </a>
            </li>
            <li class="nav-item">
                <a href="users.php" class="nav-link text-dark-emphasis">

                    <svg xmlns="http://www.w3.org/2000/svg" height="18" viewBox="0 96 960 960" width="21"><path d="M38 896v-94q0-35 18-63.5t50-42.5q73-32 131.5-46T358 636q62 0 120 14t131 46q32 14 50.5 42.5T678 802v94H38Zm700 0v-94q0-63-32-103.5T622 633q69 8 130 23.5t99 35.5q33 19 52 47t19 63v94H738ZM358 575q-66 0-108-42t-42-108q0-66 42-108t108-42q66 0 108 42t42 108q0 66-42 108t-108 42Zm360-150q0 66-42 108t-108 42q-11 0-24.5-1.5T519 568q24-25 36.5-61.5T568 425q0-45-12.5-79.5T519 282q11-3 24.5-5t24.5-2q66 0 108 42t42 108ZM98 836h520v-34q0-16-9.5-31T585 750q-72-32-121-43t-106-11q-57 0-106.5 11T130 750q-14 6-23 21t-9 31v34Zm260-321q39 0 64.5-25.5T448 425q0-39-25.5-64.5T358 335q-39 0-64.5 25.5T268 425q0 39 25.5 64.5T358 515Zm0 321Zm0-411Z"/></svg>
                  
                    ഉപയോക്താക്കൾ

                </a>
            </li>
            <li class="nav-item">
                <a href="settings.php" class="nav-link text-dark-emphasis">

                    <svg xmlns="http://www.w3.org/2000/svg" height="18" viewBox="0 96 960 960" width="21"><path d="m388 976-20-126q-19-7-40-19t-37-25l-118 54-93-164 108-79q-2-9-2.5-20.5T185 576q0-9 .5-20.5T188 535L80 456l93-164 118 54q16-13 37-25t40-18l20-127h184l20 126q19 7 40.5 18.5T669 346l118-54 93 164-108 77q2 10 2.5 21.5t.5 21.5q0 10-.5 21t-2.5 21l108 78-93 164-118-54q-16 13-36.5 25.5T592 850l-20 126H388Zm92-270q54 0 92-38t38-92q0-54-38-92t-92-38q-54 0-92 38t-38 92q0 54 38 92t92 38Zm0-60q-29 0-49.5-20.5T410 576q0-29 20.5-49.5T480 506q29 0 49.5 20.5T550 576q0 29-20.5 49.5T480 646Zm0-70Zm-44 340h88l14-112q33-8 62.5-25t53.5-41l106 46 40-72-94-69q4-17 6.5-33.5T715 576q0-17-2-33.5t-7-33.5l94-69-40-72-106 46q-23-26-52-43.5T538 348l-14-112h-88l-14 112q-34 7-63.5 24T306 414l-106-46-40 72 94 69q-4 17-6.5 33.5T245 576q0 17 2.5 33.5T254 643l-94 69 40 72 106-46q24 24 53.5 41t62.5 25l14 112Z"/></svg>

                    ക്രമീകരണങ്ങൾ
                </a>
            </li>
        </ul>
    </nav>
</div>